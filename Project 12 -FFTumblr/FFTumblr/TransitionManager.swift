...
extension TransitionManager: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        // remeber that our menuViewController will alternate between the from and to view controller depending if we're presenting or dismissing
        ...
        UIView.animate(withDuration: duration, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.8, options: [], animations: {
            if (self.presenting) {
                self.onStageMenuViewController(menuViewController) // onstage items: slide in
            }
            else {
    ...
    func offStageMenuController(_ menuViewController: MenuViewController) {
        ...
        // setup paramters for 2D transition for animations
