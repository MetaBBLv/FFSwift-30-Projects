...
class ViewController: UIViewController {
    ...
    fileprivate func loadInitialData() {
        let fileNme = Bundle.main.path(forResource: "PublicArt", ofType: "json")
        var data: Data?
        do {
            data = try Data(contentsOf: URL(fileURLWithPath: fileNme!), options: NSData.ReadingOptions(rawValue: 0))
        } catch _ {
            data = nil
        }
